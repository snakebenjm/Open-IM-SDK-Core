...
func (m *SelfMsgSyncLatestModel) syncMsg(operationID string, syncFlag int) {
	if m.seqMaxNeedSync > m.seqMaxSynchronized {
		...
		if m.seqMaxNeedSync-m.seqMaxSynchronized < uint32(constant.PullMsgNumWhenLogin) {
			...
			if m.seqMaxNeedSync-m.seqMaxSynchronized > 100 {
				m.syncMsgFromServer(m.seqMaxNeedSync-uint32(constant.PullMsgNumWhenLogin)+1, m.seqMaxNeedSync, constant.MsgSyncModelDefault, operationID)
				m.seqMaxSynchronized = m.seqMaxNeedSync
			}
